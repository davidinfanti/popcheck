...
describe("legacy listing disclosure", () => {
  ...
  it("uses the same disclosure logic in Results and generated PDFs", () => {
    ...
    const pdf = readFileSync(resolve(process.cwd(), "src/utils/generateAssessmentReport.ts"), "utf8");

    expect(results).toContain("getAnalysisSourceDisclosure");
    expect(pdf).toContain("getAnalysisSourceDisclosure(data.analysisSource)");
    expect(pdf).toContain("POPCHECK AI Assessment Report");
    expect(pdf).toContain("not a physical-item certification");
    ...
    expect(edge).toContain("Do not infer handling");
  });
});
