...
export async function generateCertificatePDF(data: CertificateData): Promise<void> {
  ...
  doc.setFontSize(22);
  doc.text("AI VERIFIED", W / 2, H / 2 + 12, { align: "center", angle: 35 });
  ...
  doc.setFont("helvetica", "bold");
  doc.setTextColor(30, 30, 30);
  doc.text("Certificate of Authenticity", W / 2, yPos, { align: "center" });
  ...
  // Verdict badge
  yPos += 8;
  const verdict = getVerdictLabel(data.score);
  ...
  try {
    ...
    doc.text("this certificate", qrX + qrSize / 2, qrY + qrSize + 7, { align: "center" });
  } catch (e) {
  ...
  doc.setFontSize(5.5);
  doc.setFont("helvetica", "normal");
  doc.text("This certificate was generated by PopCheck AI. Verification is AI-assisted and does not constitute a legal guarantee.", W / 2, H - 20, { align: "center" });
  doc.text(`© ${new Date().getFullYear()} PopCheck AI — All rights reserved`, W / 2, H - 17, { align: "center" });

  // --- Save ---
  const filename = `PopCheck_Certificate_${(data.popName || "Report").replace(/\s+/g, "_")}_${data.reportId.substring(0, 8)}.pdf`;
  doc.save(filename);
}
